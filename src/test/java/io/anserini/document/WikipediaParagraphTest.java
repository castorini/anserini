...
import io.anserini.collection.WikipediaParagraphsCollection;


public class WikipediaParagraphTest extends DocumentTest<WikipediaParagraph> {
    protected static Path tmpPath;

    @Before
    public void setUp() throws Exception {
        super.setUp();
        dType = new WikipediaParagraph("Sample Title", 0,"Sample Content");
    ...
    public void test() throws IOException {
        WikipediaParagraphsCollection wc = new WikipediaParagraphsCollection();
        Collection.FileSegment iter = wc.createFileSegment(tmpPath);
        SourceDocument parsed = iter.next();
        assertEquals(parsed.id(), "Wiktionary:Welcome, newcomers-0");
        assertEquals(parsed.content(), "This is the first paragraph.");
        parsed = iter.next();
        assertEquals(parsed.id(), "Wiktionary:Welcome, newcomers-1");
        assertEquals(parsed.content(), "This is the second paragraph.");
    }
}
