...
YahooAnswerRecParsed = collections.namedtuple('YahooAnswerRecParsed', 'uri subject content best_answ_id answ_list')
...
class YahooAnswersContentHandler(xml.sax.ContentHandler):
    ...
    def endElement(self, name):
        if name == YAWNS_DOC_TAG:
            ...
            self.worker_obj(self.qty, YahooAnswerRecParsed(self.uri, self.subject, self.content, best_answ_id, self.answ_list))
            self.qty += 1
...
class Worker:
    ...
    def __call__(self, ln, rec):
        ...
        if self.conv_qty % self.max_docs_per_file == 0:
            ...
            output_path = os.path.join(self.output_folder, 'docs{:02d}.json'.format(self.file_index))
        ...
        if self.conv_qty % 100000 == 0:
            print('Converted {} questions in {} files'.format(self.conv_qty, self.file_index))
    ...
    def finish(self):
      ...
      print('Sampling %d out of %d questions' % (self.query_sample_qty, query_qty))
      query_indx = np.random.choice(np.arange(query_qty), self.query_sample_qty)
      with open(os.path.join(self.output_folder, 'queries.tsv'), 'w') as f:
          for i in query_indx:
              f.write('%s\t%s\n' % (self.questions[i][1], self.questions[i][0]))
      with open(os.path.join(self.output_folder, 'qrels.tsv'), 'w') as f:
          for i in query_indx:
              qid = self.questions[i][0] 
              for aid, rel_grade in self.qrels[qid]:
                  f.write(qrel_entry(quest_id=qid, answ_id=aid, rel_grade=rel_grade) + '\n')
    

def convert_collection(args):
    print('Converting collection...')

    worker = Worker(args.output_folder, args.max_docs_per_file, args.query_sample_qty)

    xml.sax.parse(open_file(args.collection_path), YahooAnswersContentHandler(worker))

    worker.finish()
    

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='''Converts Yahoo Answers collection to Anserini jsonl files.''')
    parser.add_argument('--collection_path', required=True, help='Yahoo Answers file')
    parser.add_argument('--output_folder', required=True, help='output file')
    parser.add_argument('--random_seed', default=0, type=float, help='random seed')
    parser.add_argument('--query_sample_qty', type=int, required=True, help='# of queries to sample')
