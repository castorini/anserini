...
def eliminate_stop_words(stop_words, query):
  result = ''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stop_words', required=True, default='', help='stop words file path')
    ...
    with open(args.output, 'w') as fout:
      ...
      stop_words = open(args.stop_words).read().strip().split('\n')
      for line_number, line in enumerate(open(args.qid_queries)):
          query_id, query = line.strip().split('\t')
          query = eliminate_stop_words(stop_words, query)
          # query = bioclean_mod(query)
          hits = searcher.search(
              JString(query.encode('utf8')), args.hits)
