...
def evaluate_retrieval(args):
    ...
    def evaluate(query_id):
        for k in top_ks:
            if not evidences[query_id]:
                # query is labelled "NOT ENOUGH INFO"
                oracle_accuracy[k] += 1
            else:
                # query is labelled "SUPPORTS" or "REFUTES"
                for evid_set in evidences[query_id]:
                    if all([evid in predicted_docs for evid in evid_set]):
                        fully_supported[k] += 1
                        oracle_accuracy[k] += 1
                        break
    ...
    with open(args.run_file, 'r', encoding='utf-8') as f:
        curr_query = ''
        predicted_docs = {k: [] for k in top_ks}
        for line in f:
            query_id, doc_id, rank = line.strip().split('\t')

            # if we reach a new query in the run file, evaluate the previous query
            if query_id != curr_query and num_queries > 0:  # do not evaluate if no queries have been processed yet
                evaluate(query_id)
                num_queries += 1
                curr_query = query_id
                predicted_docs = {k: [] for k in top_ks}
            ...
            for k in top_ks:
                if rank <= k:
                    predicted_docs[k].append(doc_id)
    ...
    for k in top_ks:
        print(f'{k}\t{fully_supported[k] / num_queries}\t{oracle_accuracy[k] / num_queries}')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluates anserini run files in context of the FEVER task.')
