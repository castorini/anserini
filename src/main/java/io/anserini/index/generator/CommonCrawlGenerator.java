...
import io.anserini.collection.CommonCrawlDocument;
 ...
 */
public class CommonCrawlGenerator<T extends CommonCrawlDocument> implements LuceneDocumentGenerator<T> {
  ...
  public enum CommonCrawField {
    ...
    CommonCrawField(String s) {
      name = s;
    }
  }
  protected CommonCrawlGenerator() {
  }
   ...
   */
  public CommonCrawlGenerator(IndexArgs args) {
    this.args = args;
  }
  ...
  public Document createDocument(T src) throws GeneratorException {
    ...
    // Store the collection docid and url.
    document.add(new StringField(IndexArgs.ID, id, Field.Store.YES));
    document.add(new StoredField(CommonCrawField.URL.name, url));
