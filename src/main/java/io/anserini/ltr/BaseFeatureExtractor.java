    ...
    public void printFeatures(PrintStream out) throws IOException {
        ...
        for (String qid : this.qrels.getQids()) {
            ...
            for (Map.Entry<String, Integer> entry : this.qrels.getDocMap(qid).entrySet()) {
                ...
                TopDocs qrelDocs = searcher.search(docIdQuery(docId), 1);
                if (qrelDocs.totalHits.value == 0) {
                    LOG.warn(String.format("Document Id %s expected but not found in index, skipping...", docId));
                    continue;
                }

                TopDocs topDocs = searcher.search(context.getQuery(), 2);
                boolean qrelHit = false;
                for (int i = 0; i < topDocs.scoreDocs.length; i++) {
                    ...
                    if (doc.get("id").equals(docId)) {
                        qrelScore = entry.getValue();
                        qrelHit = true;
                    }

                    if (i == topDocs.scoreDocs.length - 1 && !qrelHit) {
                        hit = qrelDocs.scoreDocs[0];
                        doc = reader.document(hit.doc, fieldsToLoad);
                        qrelScore = entry.getValue();
                        qrelHit = true;
                    }
