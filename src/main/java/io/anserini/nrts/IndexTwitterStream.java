...
public class IndexTwitterStream implements Runnable {
	...
	RawStreamListener rawListener = new RawStreamListener() {
		...
		if (status == null) {
			...
			if (obj.has("delete")) {
			    ...
			    NRTSearch.indexWriter.deleteDocuments(q);
			}
		    } catch (Exception e) {
		...
		try {
		    NRTSearch.indexWriter.addDocument(doc);
		    tweetCount++;
		    if (tweetCount % 1000 == 0) {
			System.out.println(tweetCount + " statuses indexed");
		    }
		} catch (IOException e) {
		    // TODO Auto-generated catch block
		    e.printStackTrace();
