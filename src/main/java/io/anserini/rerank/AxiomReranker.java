...
public class AxiomReranker implements Reranker {
  ...
  private Set<Integer> selectDocs(ScoredDocuments docs, RerankerContext context)
    ...
    if (docidSet.size() < targetSize) {
      ...
      List<Integer> docidsPoolList = new ArrayList<>();
      for (int i = 1; i <= availableDocsCnt; i++) {
        docidsPoolList.add(i);
      }
      Random random = new Random();
      while (docidSet.size() < targetSize) {
        docidSet.add(docidsPoolList.get(random.nextInt(docidsPoolList.size())));
      }
    }
  ...
  private Map<String, Double> calTermScore(
    ...
    for (Map.Entry<String, Integer> q : queryTermsCounts.entrySet()) {
      ...
      if (termInvertedList.containsKey(queryTerm)) {
        ...
        double selfMI = calMutualInformation( termInvertedList.get(queryTerm), termInvertedList.get(queryTerm), docIdsCount );
        for (Map.Entry<String, Set<Integer>> termEntry : termInvertedList.entrySet()) {
          double score = 1.0 * qtf;
          if (!termEntry.getKey().equals(queryTerm)) { // do not include the query term itself ...
                                          // since the mutual information to itself will always be 1
            double crossMI = calMutualInformation( termInvertedList.get(queryTerm), termEntry.getValue(), docIdsCount );
  ...
  @VisibleForTesting
  private double calMutualInformation(Set<Integer> docIdsX, Set<Integer> docIdsY, int totalDocCount) {
    ...
    Set<Integer> tmp = new HashSet<>(docIdsX);
    tmp.retainAll(docIdsY);
    int numXY11 = tmp.size();
    int numXY10 = numXY10 = x1 - numXY11;	//doc num that x occure but y doesn't
    int numXY01 = y1 - numXY11;	//doc num that y occures but x doesn't
    int numXY00 = totalDocCount - numXY11 - numXY10 - numXY01; //doc num that neither x nor y occurres
