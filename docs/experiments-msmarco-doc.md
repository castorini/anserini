...
*The analysis below was generated by Codex (GPT-6).*
...
The original document grid was not preserved, so this is a replacement tuning experiment.
The final grid uses `k1=3.0:5.5:0.1` and `b=0.75:0.95:0.05`: 650 configurations across five training samples, followed by development evaluation of the averaged parameters.
...
| Training Objective | Original k1 | Original b | Newly Tuned k1 | Newly Tuned b | New MRR@100 | New MAP | New Recall@1000 |
| :----------------- | ----------: | ---------: | -------------: | ------------: | ----------: | ------: | --------------: |
| Recall@1000        |        4.46 |       0.82 |       **4.94** |      **0.86** |      0.2772 |  0.2777 |          0.9349 |
| MRR@100            |        3.80 |       0.87 |       **3.46** |      **0.88** |      0.2777 |  0.2783 |          0.9318 |
| MAP                |        3.80 |       0.87 |       **3.64** |      **0.88** |      0.2780 |  0.2786 |          0.9322 |

+ The final grid has no selected boundary winners after expanding two edges of a narrower grid.
+ The newly tuned parameters differ from the originals; their corresponding objective scores differ from the published results by -0.0008 recall, -0.0007 MRR, and -0.0003 MAP.
+ Separately, historical fixed-parameter evaluations retain differences of up to 0.0004 after rank preservation; the cause was not isolated.

For documents, the MRR cutoff is **100** and the recall cutoff is **1,000**; earlier references to MRR@10 and recall@100 in the tuning discussion should be read with these corrections.
MRR, MAP, and recall are evaluated separately from a single 1,000-hit ranking, and the new workflow selects MRR and MAP parameters independently.

*End of analysis generated by Codex (GPT-6).*
