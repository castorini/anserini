...
The most recent version of the fatjar is v2.3.0: additional information is provided in [this brief guide](release-notes/fatjar-reproduction-notes-v2.3.0.md).

Assuming you've already got Java 21 installed (Yes, you need _exactly_ this version), fetch the fatjar:

```bash
curl -fL -o anserini-2.3.0-fatjar.jar https://repo1.maven.org/maven2/io/anserini/anserini/2.3.0/anserini-2.3.0-fatjar.jar
```

Do a BM25 run on the venerable MS MARCO passage corpus using the dev queries:

```bash
java -cp anserini-2.3.0-fatjar.jar io.anserini.search.SearchCollection \
...
```bash
java -cp anserini-2.3.0-fatjar.jar trec_eval -c -M 10 -m recip_rank msmarco-v1-passage.dev \
...
```bash
java -cp anserini-2.3.0-fatjar.jar io.anserini.search.SearchHnswDenseVectors \
...
+ [Anserini v2.2.0](release-notes/fatjar-reproduction-notes-v2.2.0.md)
+ [Anserini v2.1.1](release-notes/fatjar-reproduction-notes-v2.1.1.md)
