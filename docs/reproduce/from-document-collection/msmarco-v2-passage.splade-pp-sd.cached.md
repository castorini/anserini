...
The exact configurations for these regressions are stored in [this YAML file](../../../src/main/resources/reproduce/from-document-collection/configs/msmarco-v2-passage.splade-pp-sd.cached.yaml).
...
```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --index --verify --search --regression msmarco-v2-passage.splade-pp-sd.cached
```
...
```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --download --index --verify --search --regression msmarco-v2-passage.splade-pp-sd.cached
```
