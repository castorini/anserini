...
The exact configurations for these regressions are stored in [this YAML file](../../../src/main/resources/reproduce/from-document-collection/configs/dl20-passage.bge-base-en-v1.5.parquet.hnsw.onnx.yaml).
...
```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --index --verify --search --config dl20-passage.bge-base-en-v1.5.parquet.hnsw.onnx
```
...
```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --download --index --verify --search --config dl20-passage.bge-base-en-v1.5.parquet.hnsw.onnx
```
...
bin/run.sh io.anserini.search.SearchHnswDenseVectors \
  ...
  -topics dl20 \
  -topicReader TsvInt \
  -output runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-dl20.txt \
...
```bash
bin/trec_eval -m map -c -l 2 dl20-passage runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-dl20.txt
bin/trec_eval -m ndcg_cut.10 -c dl20-passage runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-dl20.txt
bin/trec_eval -m recall.100 -c -l 2 dl20-passage runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-dl20.txt
bin/trec_eval -m recall.1000 -c -l 2 dl20-passage runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-dl20.txt
```
...
To add to this reproduction log, modify [this template](../../../src/main/resources/reproduce/from-document-collection/docgen/dl20-passage.bge-base-en-v1.5.parquet.hnsw.onnx.template) and run `bin/build.sh` to rebuild the documentation.
