...
The exact configurations for these regressions are stored in [this YAML file](../../../src/main/resources/reproduce/from-document-collection/configs/fire12-en.yaml).
Note that this page is automatically generated from [this template](../../../src/main/resources/reproduce/from-document-collection/docgen/fire12-en.template) as part of Anserini's regression pipeline, so do not modify this page directly; modify the template instead.

From one of our Waterloo servers (e.g., `orca`), the following command will perform the complete regression, end to end:

```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --index --verify --search --config fire12-en
```
...
bin/run.sh io.anserini.index.IndexCollection \
  ...
  -index indexes/lucene-inverted.fire12-en/ \
...
bin/run.sh io.anserini.search.SearchCollection \
  -index indexes/lucene-inverted.fire12-en/ \
  -topics fire12en.176-225 \
  -topicReader Trec \
  -output runs/run.lucene-inverted.fire12-en.model-bm25.topics-fire12en.176-225.txt \
  -bm25 -language en &
```
...
```bash
bin/trec_eval -m map -m P.20 -m ndcg_cut.20 fire12en.176-225 runs/run.lucene-inverted.fire12-en.model-bm25.topics-fire12en.176-225.txt
```
