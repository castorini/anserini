...
The exact configurations for these regressions are stored in [this YAML file](../../../src/main/resources/reproduce/from-document-collection/configs/msmarco-v1-passage.bge-base-en-v1.5.parquet.hnsw.onnx.yaml).
...
```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --index --verify --search --config msmarco-v1-passage.bge-base-en-v1.5.parquet.hnsw.onnx
```
...
```bash
bin/run.sh io.anserini.reproduce.ReproduceFromDocumentCollection --download --index --verify --search --config msmarco-v1-passage.bge-base-en-v1.5.parquet.hnsw.onnx
```
...
bin/run.sh io.anserini.search.SearchHnswDenseVectors \
  ...
  -topicReader TsvInt \
  -output runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-msmarco-passage.dev-subset.txt \
...
```bash
bin/trec_eval -c -m map msmarco-passage.dev-subset runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-msmarco-passage.dev-subset.txt
bin/trec_eval -c -M 10 -m recip_rank msmarco-passage.dev-subset runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-msmarco-passage.dev-subset.txt
bin/trec_eval -c -m recall.100 msmarco-passage.dev-subset runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-msmarco-passage.dev-subset.txt
bin/trec_eval -c -m recall.1000 msmarco-passage.dev-subset runs/run.lucene-hnsw.msmarco-v1-passage.bge-base-en-v1.5.model-bge-hnsw-onnx.topics-msmarco-passage.dev-subset.txt
```
...
To add to this reproduction log, modify [this template](../../../src/main/resources/reproduce/from-document-collection/docgen/msmarco-v1-passage.bge-base-en-v1.5.parquet.hnsw.onnx.template) and run `bin/build.sh` to rebuild the documentation.
