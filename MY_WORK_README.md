...
Since full compilation failed, I used existing evaluation tools:
...
**Why**: The built trec_eval had issues, so I created a custom Python script.

**Script**: `simple_trec_eval.py`
**Note**: Fixed syntax error on line 9 (`split()`g → `split()`)
